...
namespace Template.Infrastructure.Repository.Base
{
    public abstract class RepositoryEFCoreBase<T> : IRepository<T> where T : class
    {
        ...
        {
            var currentDbTx = _templateDbContext.Database.CurrentTransaction;

            if (currentDbTx == null)
            {
                var dbTx = await _templateDbContext.Database.BeginTransactionAsync(cancellationToken);
                return new UnitOfWork.UnitOfWorkTransaction(_templateDbContext, dbTx, isOuter: true, savepointName: null, onDisposeCallback: () => { });
            }
            else
            {
                var savepointName = $"sp_{Guid.NewGuid():N}";
                await currentDbTx.CreateSavepointAsync(savepointName, cancellationToken);
                return new UnitOfWork.UnitOfWorkTransaction(_templateDbContext, currentDbTx, isOuter: false, savepointName: savepointName, onDisposeCallback: () => { });
            }
        }
    }
}
